...
import webbrowser
import sys, os, random, time, calendar
...
txtfile = ""
today = date.today()
rose = "© Blake Gouthro | Rosewell OS V1.4.5 | Rosewell Software | Nov. 9th 2021 - " + str(today.year) + " | Today's Date = " + str(today) + " ©"
while True:
    print(rose)
...
#Help
        if todo == "Help" or todo == "help":
            ...
            print('Text to Speach - Opens the Text To Speach App')
...
#Text To Speech
        if todo in ['Speech','speech','SPEECH','Text To Speech','text to speech','TEXT TO SPEECH','Text to speech','text To speech','text to Speech',' Text To speech','text To Speech','Text to Speech','TTS','tts','Tts','tTs','ttS','TTs','tTS','TtS']:
...
"""
            ...
            def inputer(message="", rmessage=""):
                user = input(message)
                result = avoid if user.lower() in ["exit", "quit", "avoid",'menu','MENU','Menu','Exit','EXIT','Quit','QUIT','Q','q','M','m','E','e'] else user
                if result == avoid:
                    print(rmessage)
                return result
...
"""
                ...
                class Game:
                    ...
                    def __init__(self):
                        ...
                        self.message = "This is Where the Prompts Will Go[Type 'Menu' or 'Exit' to Stop Playing]"
                        self.gameboard = {}
                        self.placePieces()
                        print("© Blake Gouthro | Chess | V1.2.1 | Rosewell OS | 2021 - " +
                              str(today.year) + "©")
                        print(
                            "Welcome to Chess! Your Chess pieces are on the Rigt Side Automatically type in move to spaces in this format[a7 a6]. the former digit is the position of a piece, and the later digit is the Move to Position. Then you play the right side using the same digit Format. Try to Win and have FUN! You Can Only moce one piece one spot per turn")
                        self.main()
                ...
                uniDict = {WHITE: {Pawn: "♙", Rook: "♖", Knight: "♘", Bishop: "♗", King: "♔", Queen: "♕"}, BLACK: {
